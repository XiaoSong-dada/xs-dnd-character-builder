...
function wonder(id: string, name: string, description: string, rarity: NonNullable<EquipmentRule['rarity']>, equippable = true, requiresAttunement = false): EquipmentRule {
  return m({ id, name, description, category: 'magic', equippable, rarity, requiresAttunement: requiresAttunement || undefined })
}
...
export const magicItems2014: readonly EquipmentRule[] = [
  ...
  potion('potion-of-greater-healing', '强化治疗药水', '饮用后立即恢复 4d4+4 点生命值。', 'uncommon'),
  potion('potion-of-superior-healing', '强效治疗药水', '饮用后立即恢复 8d4+8 点生命值。', 'rare'),
  potion('potion-of-supreme-healing', '至尊治疗药水', '饮用后立即恢复 10d4+20 点生命值。', 'very-rare'),
  ...
  m({ id: 'mithral-armor', name: '秘银护甲', description: '以秘银合金制成的护甲：无力量需求、不施加隐蔽劣势、重量减半。可附着于任意中甲或重甲。', category: 'armor', equippable: true, rarity: 'uncommon' }),
  ...
  m({ id: 'sentinel-shield', name: '哨兵盾', description: '持握时先攻检定优势；附近 1.5 米内存在不可见生物时，你能感知其位置。', category: 'shield', equippable: true, rarity: 'uncommon', armorClassBonus: 2 }),
  m({ id: 'weapon-of-warning', name: '警示武器', description: '持有或携带时先攻检定优势；睡眠时保持警觉，不会因休息被突袭。', category: 'weapon', equippable: true, rarity: 'uncommon' }),
  ...
  wonder('bag-of-tricks', '巧技袋', '伸入袋中可取出随机动物（按袋色对应不同动物表）；动物按你指令行动，1 小时后消失。', 'uncommon', false),
  wonder('boots-of-elvenkind', '精灵靴', '行走无声：潜行（敏捷-隐匿）检定获得优势。', 'uncommon', true),
  wonder('boots-of-striding-and-springing', '大步跳跃靴', '步行速度提升至 9 米，跳跃距离增至三倍；不受负重与重甲的速度限制。', 'uncommon', true),
  ...
  wonder('brooch-of-shielding', '防护胸针', '佩戴时对魔法飞弹免疫，受到力场伤害时具有抗力。', 'uncommon', true),
  ...
  wonder('driftglobe', '漂浮光球', '可发出日光或微光照明（按口令切换），可指令其漂浮跟随。', 'uncommon', false),
  ...
  wonder('pearl-of-power', '力量之珠', '持有时可恢复一个已消耗的法术位（1–3 环中任选）。每日一次。', 'uncommon', false),
  ...
  wonder('stone-of-good-luck', '好运石', '同调后持有者的能力检定、属性检定与豁免检定 +1。', 'uncommon', true, true),
  ...
  wonder('winged-boots', '飞行靴', '同调后获得 9 米飞行速度，可悬停；每日飞行总时长 4 小时。', 'uncommon', true, true),

  // ── 法杖 / 魔杖 / 法器 ─────────────────────────────────────────
  wonder('staff-of-the-python', '巨蟒法杖', '同调后可将法杖掷出，化作巨蟒（服从命令，持续 1 小时）；巨蟒死后可恢复。', 'uncommon', true, true),
  wonder('rod-of-the-pact-keeper-+1', '契约守护法杖 +1', '术契施法者的法器：法术攻击检定与法术豁免 DC +1（需术契施法者同调）。', 'uncommon', true, true),
  wonder('wand-of-magic-detection', '侦测魔法魔杖', '可施放侦测魔法（每日次数有限）。', 'uncommon', false),
