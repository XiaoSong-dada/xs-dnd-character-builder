...
const coreFeats2014: readonly FeatRule[] = [
  ...
  feat('feat-charger', '冲锋手', 'Charger', '冲刺后可发动更具威胁的近战攻势。', '以附赠动作冲刺后，若在当回合以近战武器攻击命中目标，可额外造成 5 点伤害，或选择推撞目标使其倒地（力量豁免失败，DC 8 + 力量调整值 + 熟练加值）。', ['战斗', '移动']),
  ...
  feat('feat-inspiring-leader', '领袖之证', 'Inspiring Leader', '通过鼓舞演说为队伍提供临时生命值。', '前置：魅力 13。花费 10 分钟演说，选择至多 6 个生物（含自身）获得临时生命值 = 你的等级 + 魅力调整值；每长休可使用一次。', ['支援', '社交'], { abilityMinimum: { anyOf: ['cha'], score: 13 } }),
  ...
  feat('feat-mounted-combatant', '骑乘战斗', 'Mounted Combatant', '强化骑乘时的攻击、保护坐骑和闪避范围伤害。', '骑乘时，对体型不大于坐骑的生物发起的近战攻击获得优势；可迫使攻击坐骑的攻击改以你为目标；坐骑进行敏捷豁免成功时不受伤害、失败时只受一半伤害。', ['战斗', '骑乘']),
  ...
  feat('feat-tavern-brawler', '斗殴高手', 'Tavern Brawler', '强化徒手、临时武器和贴身擒抱。', '力量或体质 +1（上限 20）。徒手攻击与临时武器造成 1d4 + 力量调整值伤害，且视作熟练；徒手或临时武器攻击命中后，可用附赠动作尝试擒抱目标。', ['战斗', '徒手']),
...
const xgteFeats2014: readonly FeatRule[] = [
  ...
  supplementFeat('feat-dragon-fear', '龙之威怖', 'Dragon Fear', 'xgte-2017-index', '前置：龙裔。力量、体质或魅力 +1；可用吐息次数发出恐惧咖哮。', ['种族', '属性'], { prerequisite: { requiredRaceIds: ['race-2014-dragonborn'] }, choices: [abilityChoice('dragon-fear', ['str', 'con', 'cha'])] }),
  supplementFeat('feat-dragon-hide', '龙之鳞爪', 'Dragon Hide', 'xgte-2017-index', '前置：龙裔。力量、体质或魅力 +1；获得天然鳞甲与龙爪。', ['种族', '防御'], { prerequisite: { requiredRaceIds: ['race-2014-dragonborn'] }, choices: [abilityChoice('dragon-hide', ['str', 'con', 'cha'])] }),
  supplementFeat('feat-drow-high-magic', '高等卓尔魔法', 'Drow High Magic', 'xgte-2017-index', '前置：卓尔精灵。获得侦测魔法与有限次数的驱散魔法、悬浮术。', ['种族', '施法'], { prerequisite: { requiredSubraceIds: ['race-2014-elf-drow'] } }),
  ...
  supplementFeat('feat-squat-nimbleness', '低身机敏', 'Squat Nimbleness', 'xgte-2017-index', '前置：矮人或小型种族。力量或敏捷 +1；速度提升并获得运动或杂技熟练。', ['种族', '移动'], { choices: [abilityChoice('squat-nimbleness', ['str', 'dex'])] }),
...
const tcoeFeats2014: readonly FeatRule[] = [
  ...
  supplementFeat('feat-chef', '大厨', 'Chef', 'tcoe-2020-index', '体质或感知 +1；获得烹饪用具熟练，可在休息时准备恢复食物。', ['属性', '支援'], { choices: [abilityChoice('chef', ['con', 'wis'])] }),
  supplementFeat('feat-crusher', '粉碎者', 'Crusher', 'tcoe-2020-index', '力量或体质 +1；铝击命中可移动目标，暴击后盟友攻击获得优势。', ['属性', '战斗'], { choices: [abilityChoice('crusher', ['str', 'con'])] }),
  ...
  supplementFeat('feat-fey-touched', '妖精触碰', 'Fey Touched', 'tcoe-2020-index', '智力、感知或魅力 +1；习得雾步和一项 1 环占卜或惑控法术。', ['属性', '施法'], { choices: [abilityChoice('fey-touched', ['int', 'wis', 'cha'])] }),
