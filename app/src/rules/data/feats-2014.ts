...
const coreFeats2014: readonly FeatRule[] = [
  ...
  feat('feat-charger', '冲锋者', 'Charger', '冲刺后可发动更具威胁的近战攻势。', '以附赠动作冲刺后，若在当回合以近战武器攻击命中目标，可额外造成 5 点伤害，或选择推撞目标使其倒地（力量豁免失败，DC 8 + 力量调整值 + 熟练加值）。', ['战斗', '移动']),
  feat('feat-crossbow-expert', '弩专家', 'Crossbow Expert', '改善弩类武器的装填、近战射击与连贯攻击。', '装填属性对弩不再限制攻击次数；对 5 尺内生物进行远程攻击不再有劣势；以单手武器攻击后，可用附赠动作持手弩进行一次攻击。', ['战斗', '远程']),
  ...
  feat('feat-dungeon-delver', '地城探索者', 'Dungeon Delver', '强化发现与抵抗陷阱、暗门和地城危险的能力。', '搜索隐藏的暗门与陷阱的感知（察觉）与智力（调查）检定获得优势；对陷阱的豁免检定获得优势；陷阱造成的伤害获得抗性。', ['探索', '防御']),
  feat('feat-durable', '耐久', 'Durable', '提高体质，并改善使用生命骰恢复的稳定性。', '体质 +1（上限 20）。短休时掷生命骰恢复生命，最低恢复量 = 2 × 你的体质调整值（低于该值按该值计）。', ['属性', '生存']),
  feat('feat-elemental-adept', '元素精通', 'Elemental Adept', '专精一种元素伤害，使相关法术更稳定。', '前置：能施放至少一个法术。选择强酸、冷冻、火焰、闪电或雷鸣之一：该元素的伤害忽略抗性（免疫仍有效）；施放该元素法术时，伤害骰掷出 1 视为 2。', ['施法', '伤害'], { requiredCapability: 'spellcasting' }),
  ...
  feat('feat-healer', '医者', 'Healer', '强化医药包的战地救治与恢复用途。', '使用医药包稳定濒死生物时，该生物恢复 1 点生命；也可用医药包治疗生物：每只生物每长休一次，恢复 1d6+4+目标生命骰数量的生命值。', ['支援', '治疗']),
  feat('feat-heavily-armored', '重甲训练', 'Heavily Armored', '提高力量并获得重甲熟练。', '力量 +1（上限 20）。获得重甲熟练。前置：中甲熟练。', ['属性', '护甲'], { requiredCapability: 'armor-medium' }),
  ...
  feat('feat-inspiring-leader', '激励领袖', 'Inspiring Leader', '通过鼓舞演说为队伍提供临时生命值。', '前置：魅力 13。花费 10 分钟演说，选择至多 6 个生物（含自身）获得临时生命值 = 你的等级 + 魅力调整值；每长休可使用一次。', ['支援', '社交'], { abilityMinimum: { anyOf: ['cha'], score: 13 } }),
  feat('feat-keen-mind', '敏锐心灵', 'Keen Mind', '提高智力，并强化方向、时间与记忆能力。', '智力 +1（上限 20）。总能记得自己去过的方向和走过的距离；能精确感知一天内的时间；可准确回忆过去一个月内见过或听过的事情。', ['属性', '探索']),
  feat('feat-lightly-armored', '轻甲训练', 'Lightly Armored', '提高力量或敏捷，并获得轻甲熟练。', '力量或敏捷 +1（上限 20）。获得轻甲熟练。', ['属性', '护甲']),
  ...
  feat('feat-mage-slayer', '法师杀手', 'Mage Slayer', '强化贴身压制施法者和抵抗法术的能力。', '5 尺内生物施法时，可用反应进行一次近战攻击（在法术生效前）；对被你近战攻击命中的生物施放的豁免法术，其豁免检定具有劣势；施法者对你施放的专注法术被中断时，你对其的豁免获得优势。', ['战斗', '反制']),
  ...
  feat('feat-martial-adept', '武术学徒', 'Martial Adept', '学习战斗大师战技并获得一枚优势骰。', '习得两项战斗大师战技；获得一枚 d6 优势骰（随战技使用，短休或长休后恢复）。若已拥有优势骰，则其骰面提升一级。', ['战斗', '战技']),
  ...
  feat('feat-mobile', '机动', 'Mobile', '提高速度并降低高速近战后的脱离风险。', '移动速度 +10 尺；困难地形不额外消耗移动力（冲刺时）；近战攻击后，目标无法对该回合内你对它发起的借机攻击做出反应（若你攻击了多个目标则分别判定）。', ['移动', '战斗']),
  feat('feat-moderately-armored', '中甲训练', 'Moderately Armored', '提高力量或敏捷，并获得中甲与盾牌熟练。', '力量或敏捷 +1（上限 20）。获得中甲与盾牌熟练。前置：轻甲熟练。', ['属性', '护甲'], { requiredCapability: 'armor-light' }),
  feat('feat-mounted-combatant', '骑乘战斗者', 'Mounted Combatant', '强化骑乘时的攻击、保护坐骑和闪避范围伤害。', '骑乘时，对体型不大于坐骑的生物发起的近战攻击获得优势；可迫使攻击坐骑的攻击改以你为目标；坐骑进行敏捷豁免成功时不受伤害、失败时只受一半伤害。', ['战斗', '骑乘']),
  feat('feat-observant', '观察入微', 'Observant', '提高智力或感知，并强化被动察觉与调查。', '智力或感知 +1（上限 20）。被动察觉与被动调查各 +5；能读懂他人嘴唇动作（需能看见对方且对方说你能理解的语言）。', ['属性', '探索'], { abilityMinimum: { anyOf: ['int', 'wis'], score: 13 } }),
  ...
  feat('feat-resilient', '坚韧', 'Resilient', '提高一项属性，并获得对应豁免熟练。', '选择一项属性 +1（上限 20），并获得该属性的豁免熟练。', ['属性', '防御']),
  ...
  feat('feat-savage-attacker', '野蛮攻击者', 'Savage Attacker', '让近战武器伤害骰的结果更可靠。', '近战武器攻击命中时，可重掷一次伤害骰并选用较优结果；每回合一次。', ['战斗', '伤害']),
  feat('feat-sentinel', '哨兵', 'Sentinel', '强化借机攻击并限制敌人脱离前线。', '借机攻击命中时，目标移动速度降为 0；即使目标进行撤离动作，仍可对其发起借机攻击；5 尺内敌人攻击你的盟友时，可用反应进行一次近战攻击。', ['战斗', '控制']),
  ...
  feat('feat-skilled', '熟练', 'Skilled', '获得额外技能或工具熟练。', '获得三项任意技能或工具熟练（可混合选择）。', ['技能', '泛用']),
  feat('feat-skulker', '潜行者', 'Skulker', '强化隐蔽、昏暗环境与远程偷袭后的隐藏能力。', '前置：敏捷 13。对处于昏暗环境的生物进行远程攻击时，不因隐蔽劣势受影响；远程攻击未命中时不会暴露你的位置；被轻度遮蔽时可尝试躲藏。', ['探索', '潜行'], { abilityMinimum: { anyOf: ['dex'], score: 13 } }),
  feat('feat-spell-sniper', '法术狙击手', 'Spell Sniper', '强化需要攻击检定的法术射程与掩护处理。', '前置：能施放至少一个法术。需要攻击检定的法术射程翻倍；对处于部分掩护的敌人施放攻击法术不再有劣势；习得一个需要攻击检定的戏法。', ['施法', '远程'], { requiredCapability: 'spellcasting' }),
  feat('feat-tavern-brawler', '酒馆斗殴者', 'Tavern Brawler', '强化徒手、临时武器和贴身擒抱。', '力量或体质 +1（上限 20）。徒手攻击与临时武器造成 1d4 + 力量调整值伤害，且视作熟练；徒手或临时武器攻击命中后，可用附赠动作尝试擒抱目标。', ['战斗', '徒手']),
  feat('feat-tough', '强韧', 'Tough', '随等级提高生命值上限。', '生命值上限每等级 +2（含当前等级）。', ['生存', '生命']),
