...
- 角色卡新增显式编辑模式，人工差值可修改最终属性、战斗、技能/豁免、法术数值与环位，并在上游变化后自动重算。
- 支持从完整 5e-2014 系统法术库人工添加法术、一键恢复系统默认，以及角色卡、摘要、跑团助手和全部导出的统一同步。
- 草稿升级至 schema v6；跑团状态保持已损失生命和已消耗环位；全应用移动端禁用双击放大并保留捏合缩放。
...
- 导出角色卡：PDF 角色卡使用版本化三页区域映射，XLSX 自动计算卡使用 v4 字段映射与公式缓存并在打开时完整重算，JSON 数据文件使用 schema v6。PDF/XLSX 统一消费同一导出模型，阻断错误不下载、容量警告会在角色卡页显示中文摘要。
