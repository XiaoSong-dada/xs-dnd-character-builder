...
- 将草稿自动保存在当前浏览器，并支持角色 JSON 与完整角色包（ZIP，含头像/立绘）导入、导出。
- 可选角色头像与立绘：上传、焦点调整、从立绘生成头像；展示在车卡首页草稿列表、角色卡与跑团助手，图片缺失或加载失败时自动隐藏。
- 导出角色卡：PDF 角色卡使用版本化三页区域映射，XLSX 自动计算卡使用 v4 字段映射与公式缓存并在打开时完整重算，JSON 数据文件使用 schema v8（不含图片），ZIP 完整角色包包含角色数据与头像/立绘。PDF/XLSX 统一消费同一导出模型，阻断错误不下载、容量警告会在角色卡页显示中文摘要。
...
当前角色文件使用 schema v8，并可迁移导入 v2—v7 的 `5e-2014` 旧格式。浏览器存储已支持 2014 与 2024 两种草稿共存，但产品目前仅开放 2014 的创建与导入；未开放版本的导入会被拦截并给出中文提示。旧 2024 草稿继续只读隔离，不会自动转换或删除。
