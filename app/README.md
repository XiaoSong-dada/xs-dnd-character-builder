...
- 导出角色卡：PDF 角色卡使用版本化三页区域映射，XLSX 自动计算卡使用 v4 字段映射与公式缓存并在打开时完整重算，JSON 数据文件使用 schema v7（不含图片），ZIP 完整角色包包含角色数据与头像/立绘。PDF/XLSX 统一消费同一导出模型，阻断错误不下载、容量警告会在角色卡页显示中文摘要。
