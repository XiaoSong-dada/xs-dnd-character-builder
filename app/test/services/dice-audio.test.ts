...
describe('dice audio', () => {
  it('is lazy and tolerates unsupported audio', () => {
    ...
    expect(() => { audio.unlock(); audio.play(100, 1); audio.dispose() }).not.toThrow()
  })
  ...
  it('stops scheduled impacts and closes resources, including after playback failure', () => {
    ...
    expect(() => audio.play(500, 20)).not.toThrow()
    audio.dispose()
  ...
  it('handles blocked resume without an unhandled rejection', async () => {
    ...
    expect(() => audio.play(200, 1)).not.toThrow()
    audio.dispose()
  })

  it('cannot restart sound when resume resolves after disposal', async () => {
    let finish: (() => void) | undefined
    const createSource = vi.fn()
    const close = vi.fn(() => Promise.resolve())
    const context = {
      state: 'suspended', close, createBufferSource: createSource,
      resume: () => new Promise<void>((resolve) => { finish = resolve }),
    }
    vi.stubGlobal('AudioContext', function () { return context })
    const audio = new DiceAudioService()
    audio.unlock()
