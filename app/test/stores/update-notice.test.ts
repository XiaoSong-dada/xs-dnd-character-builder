...
describe('useUpdateNoticeStore', () => {
  ...
  it('首次访问弹出，关闭后写入当前版本', () => {
    ...
    expect(store.notice?.version).toBe(__APP_VERSION__)

    store.dismiss()
    expect(store.isOpen).toBe(false)
    expect(JSON.parse(localStorage.getItem(UPDATE_NOTICE_STORAGE_KEY) ?? '{}')).toEqual({ lastSeenVersion: __APP_VERSION__ })
  })

  it('相同版本不弹，不同版本弹出，初始化保持幂等', () => {
    localStorage.setItem(UPDATE_NOTICE_STORAGE_KEY, JSON.stringify({ lastSeenVersion: __APP_VERSION__ }))
    const store = useUpdateNoticeStore()
    store.initialize()
    expect(store.isOpen).toBe(false)
