...
describe('useUpdateNoticeStore', () => {
  ...
  it('首次访问弹出，关闭后写入当前版本', () => {
    ...
    expect(store.notice?.version).toBe('1.1.2')

    store.dismiss()
    expect(store.isOpen).toBe(false)
    expect(JSON.parse(localStorage.getItem(UPDATE_NOTICE_STORAGE_KEY) ?? '{}')).toEqual({ lastSeenVersion: '1.1.2' })
  })

  it('相同版本不弹，不同版本弹出，初始化保持幂等', () => {
    localStorage.setItem(UPDATE_NOTICE_STORAGE_KEY, JSON.stringify({ lastSeenVersion: '1.1.2' }))
    const store = useUpdateNoticeStore()
    store.initialize()
    expect(store.isOpen).toBe(false)
