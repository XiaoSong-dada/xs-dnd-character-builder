...
describe('versioned rules repositories', () => {
  ...
  it('resolves the versioned 2024 repository', () => {
    ...
    // 2024 可切换来源目前分为四类：游玩测试（UA）、旧扩展镜像、第三方合作内容与官方扩展（G2 批次起）。
    expect(repository.sources.filter((source) => source.selectable)
      .every((source) => source.contentKind === 'playtest' || source.contentKind === 'legacy' || source.contentKind === 'third-party' || source.contentKind === 'official')).toBe(true)
  })
