...
| G-F2 | 第三方 2024 写法背景 18 条（CM 13 + DoD 4 + CBT 1）与装备映射 | 18 | 待办 |
...
**测试加固（跨小批）**：`origins-2014`／`race-features-2014`／`starting-equipment-2014`／`SourcesStep`／`version-aware-steps-2024`／`source-books`／`repositories` 的计数断言统一改为**集合覆盖或下界**断言，后续小批不再需要改测试。
