...
- 角色卡新增显式编辑模式，支持最终属性、战斗、技能/豁免、法术数值与 1—9 环法术位人工修正；上游变化会自动重算依赖值。
- 可从完整 5e-2014 系统法术库人工添加法术，并同步角色卡、摘要、跑团助手和 PDF/XLSX/JSON 导出。
- 草稿升级为 schema v6，支持 v2—v5 自动迁移、一键恢复系统默认及跑团生命/环位状态协调。
- 全应用移动端禁用双击放大，同时保留滚动与双指捏合缩放。
